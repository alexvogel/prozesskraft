...
implements Serializable, Cloneable
{
	...
	public void genCode(String type)
	{
		...
		if(type.matches("process"))
		{
			...
			content.add("$id = $instancedir;");
			content.add("$id =~ s/^.+_(\\d+_\\d+)$/$1/;");
			content.add("");
		...
		else
		{
			...
			content.add("# getConfigKeys(\"^salut_\") to obtain all keys used in the config file that start with \"^salut_\". You may use any pattern.");
			content.add("#");
