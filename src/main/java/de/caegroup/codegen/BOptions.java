...
implements Serializable, Cloneable
{
	...
	public void genCode(String type)
	{
		...
		if(type.matches("process"))
		{
			...
			content.add("	$OPTIONS_TABLE{'conf'} = {'reihenfolge' => '10', 'minoccur' => '0', 'maxoccur' => '99', 'definition' => 'string', 'check'=>'^[^=]+=[^=]+$', 'default' => '', 'allowIntegratedListIfMultiOption' => 'false', 'text1' => '=KEY=VALUE', 'text2' => 'for redefinition of configuration variables (instead of using the ones in '.Cwd::realpath(File::Spec->rel2abs($conf_path1)).'). (possible KEYs are: ' . join(\", \", sort keys %CONF_TMP) . ')'};");
			content.add("}");
			content.add("elsif (stat $conf_path2)");
			content.add("{");
			content.add("	my %CONF_TMP = &getvars($conf_path2);");
			content.add("	$OPTIONS_TABLE{'conf'} = {'reihenfolge' => '10', 'minoccur' => '0', 'maxoccur' => '99', 'definition' => 'string', 'check'=>'^[^=]+=[^=]+$', 'default' => '', 'allowIntegratedListIfMultiOption' => 'false', 'text1' => '=KEY=VALUE', 'text2' => 'for redefinition of configuration variables (instead of using the ones in '.Cwd::realpath(File::Spec->rel2abs($conf_path2)).').\n(possible KEYs are: ' . join(\", \", sort keys %CONF_TMP) . ')'};");
			content.add("}");
		...
		else
		{
			...
			content.add("if (stat $doc_path)");
			content.add("{");
			content.add("	#$OPTIONS_TABLE{'doc'} = {'reihenfolge' => '9', 'minoccur' => '0', 'maxoccur' => '1', 'definition' => 'flag', 'check'=>'', 'default' => '', 'allowIntegratedListIfMultiOption' => 'false', 'text1' => '', 'text2' => \"shows documentation(\".Cwd::realpath(File::Spec->rel2abs($doc_path)).\")\"};");
			content.add("	$OPTIONS_TABLE{'doc'} = {'reihenfolge' => '9', 'minoccur' => '0', 'maxoccur' => '1', 'definition' => 'flag', 'check'=>'', 'default' => '', 'allowIntegratedListIfMultiOption' => 'false', 'text1' => '', 'text2' => \"shows documentation\"};");
			content.add("}");
			content.add("# wenn kein config-file vorhanden ist, wird auch diese option nicht angelegt");
			content.add("if (stat $conf_path1)");
			content.add("{");
			content.add("	my %CONF_TMP = &getvars($conf_path1);");
			content.add("	$OPTIONS_TABLE{'conf'} = {'reihenfolge' => '10', 'minoccur' => '0', 'maxoccur' => '99', 'definition' => 'string', 'check'=>'^[^=]+=[^=]+$', 'default' => '', 'allowIntegratedListIfMultiOption' => 'false', 'text1' => '=KEY=VALUE', 'text2' => 'for redefinition of configuration variables (possible KEYs are: ' . join(\", \", sort keys %CONF_TMP) . ')'};");
			content.add("}");
			content.add("elsif (stat $conf_path2)");
			content.add("{");
			content.add("	my %CONF_TMP = &getvars($conf_path2);");
