...
implements Serializable, Cloneable
{
	...
	public void genCode(String type) {
		...
		else
		{
			...
			content.add("			if ( (${$OPTHELP{$_}}{'default'} eq \"true\") && (${$OPTIONS_TABLE{$_}}{'definition'} eq \"flag\") )");
			content.add("			{");
			content.add("				$helptext .= \", default: on)\";");
			content.add("			}");
			content.add("			# wenn der default==false und es sich um einen flag-parameter handelt");
			content.add("			elsif ( (${$OPTHELP{$_}}{'default'} eq \"false\") && (${$OPTIONS_TABLE{$_}}{'definition'} eq \"flag\") )");
			content.add("			{");
