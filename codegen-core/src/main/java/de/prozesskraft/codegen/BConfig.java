...
implements Serializable, Cloneable
{
	...
	public void genCode(String type)
	{
		...
		else
		{
			...
			content.add("			if (&getConfig($param[0]))");
			content.add("			{");
			content.add("				logit(\"warn\", \"reconfiguring parameter (\" . $param[0] . \"=\" . &getConfig($param[0]) . \") with user value (\" . $param[0] . \"=\" . $param[1] . \")\");");
			content.add("				&getConfig($param[0]) = $param[1];");
			content.add("			}");
			content.add("			else");
			content.add("			{");
			content.add("				logit(\"fatal\", \"user config parameter does not exist in configfile (\" . $param[0] . \") - ignoring.\");");
			content.add("				logit(\"info\", \"you only may use these keys as a configuration parameter: \" . join (\", \", &getConfigKeys()));");
			content.add("				exit(1);");
			content.add("			}");
			content.add("		}");
			content.add("	}");
